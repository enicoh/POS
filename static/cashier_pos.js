...
function applySettings(settings) {
  ...
  if (settingsMap["primary-color"]) {
    document.documentElement.style.setProperty(
      "--primary-color",
      settingsMap["primary-color"],
    );
  }
  if (settingsMap["secondary-color"]) {
    document.documentElement.style.setProperty(
      "--secondary-color",
      settingsMap["secondary-color"],
    );
  }
  if (settingsMap["accent-color"]) {
    document.documentElement.style.setProperty(
      "--accent-color",
      settingsMap["accent-color"],
    );
  }

  // Apply background
  if (
    settingsMap["background-type"] === "solid" &&
    settingsMap["background-color"]
  ) {
    document.querySelector(".pos-container").style.background =
      settingsMap["background-color"];
  } else if (
    settingsMap["background-type"] === "gradient" &&
    settingsMap["primary-color"] &&
    settingsMap["secondary-color"]
  ) {
    document.querySelector(".pos-container").style.background =
      `linear-gradient(135deg, ${settingsMap["primary-color"]}, ${settingsMap["secondary-color"]})`;
  }
...
function displayProducts(productsToShow) {
  ...
  if (productsToShow.length === 0) {
    productGrid.innerHTML =
      '<div class="col-12 text-center text-white"><p>No products available</p></div>';
    return;
  }
...
function showProductModal(productId) {
  ...
  document.getElementById("modalProductImage").src =
    selectedProduct.image_url || "/static/placeholder-coffee.svg";
  ...
  if (selectedProduct.sizes && selectedProduct.sizes.length > 0) {
    ...
    sizeButtons.innerHTML = selectedProduct.sizes
      .map(
        (size) => `
  ...
  if (selectedProduct.modifiers && selectedProduct.modifiers.length > 0) {
    ...
    modifierList.innerHTML = selectedProduct.modifiers
      .map(
        (modifier) => `
...
function updateQuantityButtons() {
  const increaseBtn = document.querySelector(
    '.quantity-btn[onclick="increaseQuantity()"]',
  );
  const decreaseBtn = document.querySelector(
    '.quantity-btn[onclick="decreaseQuantity()"]',
  );
...
function addToOrder() {
  ...
  const existingItemIndex = currentOrder.findIndex(
    (item) =>
      ...
      JSON.stringify(item.modifier_ids.sort()) ===
        JSON.stringify([...selectedModifiers].sort()) &&
  ...
  if (existingItemIndex !== -1) {
    ...
    currentOrder[existingItemIndex].total_price =
      currentOrder[existingItemIndex].unit_price *
    ...
    const orderItem = {
      ...
      size_name: selectedSize
        ? selectedProduct.sizes.find((s) => s.id === selectedSize)?.name
        : null,
      ...
      modifiers: selectedModifiers
        .map((id) => {
          ...
          return modifier
            ? { name: modifier.name, price_modifier: modifier.price_modifier }
            : null;
        })
        .filter((m) => m),
...
function updateOrderDisplay() {
  ...
  // Display order items
  orderItems.innerHTML = currentOrder
    .map(
      (item, index) => `
  ...
  amountInput.addEventListener("input", function () {
    ...
    if (change >= 0) {
      ...
      changeDisplay.className =
        "form-control-plaintext fw-bold fs-3 text-success";
    } else {
      processBtn.disabled = true;
      changeDisplay.className =
        "form-control-plaintext fw-bold fs-3 text-danger";
    }
  });
...
async function processCashPayment(amountReceived) {
  ...
  try {
    ...
    generateReceipt(orderForReceipt, paymentInfo);

    // Short delay to allow DOM to update before printing
    setTimeout(() => {
      window.print();
...
async function showPendingOrders() {
  try {
    ...
    const modal = new bootstrap.Modal(
      document.getElementById("pendingOrdersModal"),
    );
    modal.show();
  } catch (error) {
...
// Receipt Generation
function generateReceipt(order, payment) {
  const container = document.getElementById("receipt-container");
  const date = new Date().toLocaleString();

  // Explicitly check for shop name element
  const shopNameElement = document.querySelector(".pos-header h3"); // Changed from h1 to h3 based on HTML file
  const shopName = shopNameElement
    ? shopNameElement.innerText.trim()
    : "Coffee Shop POS";
    ...
    if (item.modifiers && item.modifiers.length > 0) {
      item.modifiers.forEach((mod) => {
        html += `
                    <tr>
                        <td></td>
                        <td style="font-size: 9px; padding-left: 5px;">+ ${mod.name}</td>
                        <td class="text-end" style="font-size: 9px;">${mod.price_modifier.toFixed(2)}</td>
                    </tr>
                `;
      });
    }
  ...
  html += `
        ...
        <div class="receipt-totals">
            <div class="receipt-total-line">
                Subtotal: DZD ${order.total_price.toFixed(2)}
            </div>
            <div class="receipt-total-line">
                Payment (${payment.method}): DZD ${payment.amount_received.toFixed(2)}
            </div>
            ${
              payment.change > 0
                ? `
            <div class="receipt-total-line">
                Change: DZD ${payment.change.toFixed(2)}
            </div>
            `
                : ""
            }
            <div class="receipt-grand-total">
                Total: DZD ${order.total_price.toFixed(2)}
            </div>
        </div>
