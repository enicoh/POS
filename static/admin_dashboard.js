  ...
  try {
    ...
    try {
      ...
      if (inventory.low_stock_products.length === 0) {
        ...
        lowStockList.innerHTML = inventory.low_stock_products
          .map(
            (product) => `
...
async function loadInventory() {
  try {
    ...
    tableBody.innerHTML = inventory.inventory
      .map((product) => {
        let statusClass = "";
        let statusText = "";
...
async function loadProducts() {
  try {
    ...
    tableBody.querySelectorAll("tr").forEach((row, index) => {
      if (index % 2 === 0) {
        // Only for product rows, not detail rows
...
function showAddProductModal() {
  ...
  document.getElementById("size-inputs-container").innerHTML =
    '<div class="text-muted small text-center py-2" id="no-sizes-msg">No sizes added</div>';
  document.getElementById("modifier-inputs-container").innerHTML =
    '<div class="text-muted small text-center py-2" id="no-modifiers-msg">No modifiers added</div>';
...
async function addProduct() {
  try {
    ...
    // Collect sizes
    const sizeNames = document.getElementsByName("size_name[]");
    const sizePrices = document.getElementsByName("size_price[]");
    const sizes = [];
    for (let i = 0; i < sizeNames.length; i++) {
      if (sizeNames[i].value) {
        sizes.push({
          name: sizeNames[i].value,
          price_modifier: parseFloat(sizePrices[i].value) || 0,
        });
      }
    }
    if (sizes.length > 0) {
    ...
    // Collect modifiers
    const modifierNames = document.getElementsByName("modifier_name[]");
    const modifierPrices = document.getElementsByName("modifier_price[]");
    const modifiers = [];
    for (let i = 0; i < modifierNames.length; i++) {
      if (modifierNames[i].value) {
        modifiers.push({
          name: modifierNames[i].value,
          price_modifier: parseFloat(modifierPrices[i].value) || 0,
        });
      }
    }
    ...
    const msg =
      error && error.message ? error.message : "Failed to add product";
...
async function resetDailyAnalytics() {
  if (
    !confirm(
      "Are you sure you want to reset daily analytics? This will clear today's data from the analytics view but preserve it for PDF reports.",
    )
  ) {
    return;
  }
        ...
        if (result.next_reset) {
          ...
          if (timeDiff > 0) {
            ...
            messageElement.textContent =
              "Next automatic reset: Tonight at 00:00";
          }
        } else {
          messageElement.textContent =
            "Automatic reset scheduled for 00:00 every day";
        }
      } else {
        messageElement.textContent = "Scheduler not active";
      }
    } else {
      document.getElementById("scheduler-message").textContent =
...
async function saveModifierInline(productId, name, priceModifier) {
  try {
    const response = await fetch(
      `/api/pos/admin/products/${productId}/modifiers`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("authToken")}`,
...
async function updateModifierInline(
  modifierId,
  name,
  priceModifier,
  productId,
...
async function editProduct(productId) {
  try {
    ...
    if (!product) {
      product = inventory.inventory.find(
    ...
    setTimeout(() => {
      ...
      if (categoryEl) {
        console.log(
          ...
          Array.from(categoryEl.options).map((opt) => ({
            value: opt.value,
            text: opt.text,
          })),
        );
        console.log(
          "Trying to set category to:",
          product.category_id,
          "Type:",
    ...
    if (buttonEl) {
      ...
      console.log(
        "Button text changed to: Update Product, onclick set to updateProduct",
      );
    }
  } catch (error) {
...
async function editCategory(categoryId) {
  try {
    ...
    console.log(
      ...
      categories.map((c) => ({ id: c.id, name: c.name, type: typeof c.id })),
    );
...
// Helper functions for dynamic size/modifier inputs
function addSizeInput() {
  ...
  div.id = `size-row-${id}`;
  div.innerHTML = `
        <input type="text" class="form-control" placeholder="Size Name (e.g. Large)" name="size_name[]" required>
        <span class="input-group-text">Price +</span>
        <input type="number" class="form-control" placeholder="0" name="size_price[]" value="0" min="0" required>
        <button type="button" class="btn btn-outline-danger" onclick="removeDynamicInput('size-row-${id}')">
...
function addModifierInput() {
  const container = document.getElementById("modifier-inputs-container");
  const noModsMsg = document.getElementById("no-modifiers-msg");
  if (noModsMsg) noModsMsg.style.display = "none";

  const id = Date.now();
  const div = document.createElement("div");
  div.className = "input-group mb-2";
  div.id = `modifier-row-${id}`;
  div.innerHTML = `
        <input type="text" class="form-control" placeholder="Modifier Name (e.g. Extra Cheese)" name="modifier_name[]" required>
        <span class="input-group-text">Price +</span>
        <input type="number" class="form-control" placeholder="0" name="modifier_price[]" value="0" min="0" required>
        <button type="button" class="btn btn-outline-danger" onclick="removeDynamicInput('modifier-row-${id}')">
            <i class="fas fa-times"></i>
        </button>
    `;
  container.appendChild(div);
...
function removeDynamicInput(id) {
  const element = document.getElementById(id);
  if (element) {
    element.remove();
    // Check if containers are empty and show msg
    const sizeContainer = document.getElementById("size-inputs-container");
    if (sizeContainer && sizeContainer.children.length <= 1) {
      const noSizesMsg = document.getElementById("no-sizes-msg");
      if (noSizesMsg) {
    ...
    const modContainer = document.getElementById("modifier-inputs-container");
    if (modContainer && modContainer.children.length <= 1) {
      const noModsMsg = document.getElementById("no-modifiers-msg");
      if (noModsMsg) {
